...
//public:
   Trie:: Trie() { root = new Node(); }
    ...
    int Trie:: search(string word)//поиск количества вхождений данного слова
    {
        if (word == " ")
            return;
        Node* node = root;
    ...
    void Trie::Delete(string word)// метод удаления слова целиком
    {
        if (word == "")
            return;
        Node* node = root;
    ...
    Node* Trie::remove(Node* root, string key, int depth = 0)// удаление ключа из дерева
    {
        if (!root)
            return nullptr;
        if (depth == key.length())
    ...
    void Trie:: Print(Node* node)//печать отдельного узла
    {
        if (node == nullptr)
            return;
        if (node->Controlword != "")
    ...
    void Trie:: printPre(string Prefix)//печать префикса или слова целиком
    {
        if (Prefix== "")
            return;
        Node* node = root;
